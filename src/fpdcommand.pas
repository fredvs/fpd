...
begin
  ...
  if trim(AParams) = '' then
    WriteLN('No filename entered') else
  if not fileexists(AParams) then
    WriteLN('File does not exist: ' + AParams) else
   begin
     GController.ExecutableFilename := AParams;
     CallProcessLoop:=false;
...
procedure HandleInit(AParams: String; out CallProcessLoop: boolean);
var
...
begin
  ...
  WriteLN('The debuggee is already running') else
  begin
    CallProcessLoop:=false;
    if not assigned(GController.MainProcess) then
    begin
      WriteLN('The debuggee was not initialized');
      Exit;
    end else WriteLN('Running ...');
    IniState := isRun;
    CallProcessLoop:=true;
  end;
end;
...
begin
  ...
  WriteLN('The process is not running') else
  begin
  CallProcessLoop:=false;
...
begin
  ...
  MCommands.AddCommand(['init', 'i'], @HandleInit,  'init: Initialize the debuggee');
  ...
  MCommands.AddCommand(['step', 's'], @HandleStep,  'step: Steps one line into procedure');
